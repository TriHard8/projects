...
int main(int argc, char **argv){
    ...
    for(auto it = options.begin(); it != options.end(); ++it){
        ...
        //if((*it)->getVolCall() > 2 * (*it)->getOpenIntCall() && (*it)->getVolCall() > mini && (*it)->getVolCall() < maxi){
            cout << i << ": ";
            cout << "Call -> " << (*it)->getSymbol() << " : " << (*it)->getExpiration() << "( " << (*it)->getCurrent() << " : " << (*it)->getStrike() << " ) : " << (*it)->getVolCall() << "( " << (*it)->getOpenIntCall() << " )" << endl;
            ++i;
        }
        if((*it)->getUpDown() && (*it)->getVolPut() > 2 * (*it)->getOpenIntPut() && (*it)->getVolPut() > mini && (*it)->getVolPut() < maxi){
        //if((*it)->getVolPut() > 2 * (*it)->getOpenIntPut() && (*it)->getVolPut() > mini && (*it)->getVolPut() < maxi){
            cout << i << ": ";
