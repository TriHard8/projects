...
class Player{
    ...
    public:
        Player() {};
        ~Player() {};
        Player(int newSalary, std::string newNameID, std::string newName, std::string newMatchup, std::string newTeam, std::string newID, int newOdds, float newPoints, int newQualifying):
                                    salary(newSalary),
                                    nameid(newNameID),
                                    name(newName),
                                    matchup(newMatchup),
	...
	void setQualifying(unsigned int newQualifying) { qualifying = newQualifying; };
	...
	int getQualifying() const { return qualifying; };
...
void DKSlate::readRecordsDK(std::string file){ 
	...
	if(site == 1){ 
        ...
        //Player(int newSalary, std::string newNameID, std::string newName, std::string newMatchup, std::string newTeam, std::string newID, int newOdds, float newPoints, unsigned long newQualifying):

        Player* player;
        if(site == 1)
            player = new Player(std::stoi(row[5]),row[1], row[2], matchup, row[7], row[3],std::stoi(row[9]),std::stof(row[10]), std::stoul(row[14]));
        else if(site == 2)
            player = new Player(std::stoi(row[7]),row[3] + " (" + row[1] + ")", row[3], matchup, row[9], row[1], std::stoi(row[14]), std::stoi(row[16]), std::stoul(row[18]));
    ...
    if(sport == 2){
	int count(1);
	std::vector<Player*> foo(players.size());
	std::sort(players.begin(), players.end(), [](Player* &p1, Player* &p2){ return (p1->getOdds() < p2->getOdds()); } );
	auto it = std::copy_if(players.begin(), players.end(), foo.begin(), [&count](Player* newPlayer){
	    if(count < newPlayer->getQualifying()) std::cout << newPlayer->getName() << " - " << count-newPlayer->getQualifying() << std::endl;
	    return (count++ < newPlayer->getQualifying());
	} );
	foo.resize(std::distance(foo.begin(), it));
	//std::cout << foo.size() << std::endl;	
	//std::for_each(foo.begin(), foo.end(), [](Player* &newPlayer){ std::cout << newPlayer->getName() << " : " << newPlayer->getSalary() << std::endl; });	
...
void DKSlate::make_combos(){
    ...
    unsigned int upperSalary, lowerSalary;
    int k;
    if (site == 1){
        k = 6; //This is the setting for tennis/golf/nascar/mma on draftkings (i.e. 6 players in the slate's lineup)
        upperSalary = 50000;
        lowerSalary = 45000;
    }
    else if(site == 2){
...
*/
    ...
    while(combo < ones){
        ...
        if(sport == 1) matches = head2head(combo);
	else if(sport == 2) matches = false;
