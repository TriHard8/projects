...
class Player{
    ...
    public:
        Player() {};
        ~Player() {};
        Player(int newSalary, std::string newNameID, std::string newName, std::string newMatchup, std::string newTeam, std::string newID, int newOdds, float newPoints, std::string newPosition):
                                    salary(newSalary),
                                    nameid(newNameID),
                                    name(newName),
                                    matchup(newMatchup),
...
class DKSlate{
    private:
        ...
        std::set<Player*> goalies;
        std::set<Player*> defensemen;
        std::set<Player*> wingers;
        std::set<Player*> centers;
        std::set<Player*> utility;
...
void DKSlate::readRecordsDK(std::string file){ 
    // File pointer 
    std::fstream fin; 
    std::string path, filename,rotoStart,DKScore;
    std::set<std::string> playersPlaying;
    std::set<std::string> slate;
    std::set<std::string> playerCheck;
    ...
    DKScore = path + "DKscoringOdds.csv";
    DKMapping(DKtoRoto);
    ...
    while(getline(fin, line)) { 
        ...
        //Player(int newSalary, std::string newNameID, std::string newName, std::string newMatchup, std::string newTeam, std::string newID):
        ...
        if(site == 1){
            ...
            player = new Player(std::stoi(row[7]),row[3] + " (" + row[1] + ")", row[3], matchup, row[9], row[1], row[0]);
        }
        ...
        if(playersPlaying.find(player->getName()) == playersPlaying.end()){
            ...
            if(row[0].find('G') != std::string::npos){
                goalies.insert(player);
            }
            else{
                utility.insert(player);
                if(row[0].find('W') != std::string::npos) wingers.insert(player);
                else if(row[0].find('C') != std::string::npos) centers.insert(player);
                else if(row[0].find('D') != std::string::npos) defensemen.insert(player);
            }
        }
    ...
    std::for_each(playersPlaying.begin(), playersPlaying.end(), [&slate,&diff](std::string a){ if(slate.find(a) == slate.end()) { std::cout << a << std::endl; diff.push_back(a); }});
    if(diff.size() != 0){
    ...
    std::cout << playerCheck.size() << " : " << slate.size() << std::endl;
    
    std::cout << "Goalies: " << goalies.size() << std::endl;
    std::cout << "Wingers: " << wingers.size() << std::endl;
    std::cout << "Defensemen: " << defensemen.size() << std::endl;
    std::cout << "Centers: " << centers.size() << std::endl;
    std::cout << "Utility: " << utility.size() << std::endl;
    
    //std::for_each(wingers.begin(), wingers.end(), [](Player *newPlayer){ std::cout << newPlayer->getName() << std::endl; });
